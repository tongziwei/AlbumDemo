 ...
 */
package com.goertek.albumdemo.data;
...
import com.goertek.albumdemo.Filter;
...
public class MediaReader {
    ...
    public MediaReader(Context mContext) {
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
               ...
               /* if (mSizeFilter != null && mSizeFilter.filter(size)) {
                ...
                List<AlbumFile> collectedFile = DataSupport.where("mPath = ? ",path).find(AlbumFile.class);  //判断该文件是否收藏
                if(collectedFile.size()!=0){
                    imageFile.setCollected(true);
                }else{
                    imageFile.setCollected(false);
                }
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
              ...
              /*  if (mSizeFilter != null && mSizeFilter.filter(size)) {
                ...
                List<AlbumFile> collectedFile = DataSupport.where("mPath = ? ",path).find(AlbumFile.class);
                if(collectedFile.size()!=0){
                    videoFile.setCollected(true);
                }else{
                    videoFile.setCollected(false);
                }
